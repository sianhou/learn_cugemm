    ...
    for (int kk = 0; kk < K; kk += STEP) {
        ...
        for (int j = 0; j < STRIDE; ++j) {
            for (int i = 0; i < STRIDE; ++i) {
                shared_a[ty * STRIDE + j][tx * STRIDE + i] = ptr_a[(ty * STRIDE + j) * K + tx * STRIDE + i];
                shared_a[ty * STRIDE + j][tx * STRIDE + i] = ptr_b[(ty * STRIDE + j) * K + tx * STRIDE + i];
                __syncthreads();
            }
        }
...
#pragma unroll
        for (int b = 0; b < BLK; ++b) {
            for (int j = 0; j < STRIDE; ++j) {
                for (int i = 0; i < STRIDE; ++i) {
                    sum[j][i] += shared_a[ty * STRIDE + j][b + i] * shared_b[b + j][tx * STRIDE + i];
                }
            }
        }
        __syncthreads();
    ...
    for (int j = 0; j < STRIDE; ++j) {
        for (int i = 0; i < STRIDE; ++i) {
            c[(STEP * by + ty * STRIDE + j) * N + STEP * bx + tx * STRIDE + i] = sum[j][i];
        }
    }
}
...
//    std::cout << " ----------- warmup() ----------- " << std::endl;
    cu_sgemm<BLKY, 2><<<grid, block>>>(d_A, d_B, d_C, M, N, K);
